    ...
    <div className="h-screen flex justify-center items-center ">
        ...
        <div className="w-11/12 md:w-6/12  flex justify-center items-center ">
            <div className="h-full w-11/12 flex flex-col justify-center items-center">
                ...
                <h1 className="w-11/12 text-wrap text-start text-xl mb-2 font-semibold">Login to your account as admin</h1>
                <p className="w-11/12 text-wrap text-start text-sm m-3 font-light">Its nice to see you again.</p>
                <form type="submit" className="w-full flex justify-center flex-col items-center gap-5" onSubmit={handleLogin}>
                    <input type="email" placeholder="Your email" className="w-11/12 rounded-md outline-none p-3 bg-white" style={{border:"1px solid gray"}} value={admin.email} 
                    onChange={(e)=>setAdmin({...admin,email: e.target.value})}/>
                    <input type="password" placeholder="Your password" className="w-11/12 rounded-md outline-none p-3 bg-white" style={{border:"1px solid gray"}} value={admin.password} 
                    onChange={(e)=>setAdmin({...admin,password: e.target.value})}/>
                    <button type="submit" className="w-10/12 mt-10 md:m-0 text-2xl md:text-lg bg-blue-700 py-4 px-8 md:py-3 md:px-6 text-white rounded-lg">Login</button>
                </form>
            </div>
        </div>
    </div>
